...
@section('content')
<div class="space-y-8 pb-20" x-data="flaggedManager({{ json_encode($users->items()) }}, {{ json_encode(['current_page' => $users->currentPage(), 'last_page' => $users->lastPage(), 'total' => $users->total()]) }})">
                            ...
                            <td class="pl-4 pr-2 sm:px-8 py-4 sm:py-6">
                                <div class="flex items-center gap-3 sm:gap-4 overflow-hidden">
                                    <div class="relative shrink-0">
                                        <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-red-50 border border-red-100 flex items-center justify-center grayscale group-hover:grayscale-0 transition-all shadow-sm overflow-hidden">
                                            <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=EF4444&color=fff&size=128`" 
                                                 class="w-full h-full object-cover">
                                        </div>
                                        <div class="absolute -top-1 -right-1 w-4 sm:w-5 h-4 sm:h-5 bg-rdc-red text-[7px] sm:text-[8px] flex items-center justify-center text-white rounded-full border border-white font-black animate-pulse">!</div>
                                    </div>
                                    <div class="min-w-0">
...
<script>
function flaggedManager(initialUsers = [], initialPagination = {}) {
    return {
        users: initialUsers,
        loading: false,
