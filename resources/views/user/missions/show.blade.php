...
<div class="space-y-10 pb-20 max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-sm relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div class="relative z-10 text-center md:text-right">
            ...
            @elseif($mission->status == 'in_progress' && $mission->serviceRequest?->status === 'awaiting_validation')
                ...
                <div class="text-[10px] font-black text-amber-500 uppercase tracking-widest">À valider</div>
    ...
    {{-- Commission already paid badge (historical — no new commission payments
         can be initiated in-app anymore; everything is settled in cash between
         client and artisan, outside the platform) --}}
    ...
    <div class="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl">
        ...
        @if($mission->status == 'in_progress' && $mission->serviceRequest?->status === 'awaiting_validation')
        <div class="w-full flex items-center gap-3 px-6 py-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-amber-300 text-xs font-bold">
            <i class="fas fa-hourglass-half"></i> En attente de la confirmation du client — vous serez notifié dès qu'il aura validé.
        </div>
        @else
        <form action="{{ route('user.missions.update-status', $mission->id) }}" method="POST" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <button type="submit" class="w-full md:w-auto px-10 py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 justify-center" onclick="return confirm('Signaler ce travail comme terminé ? Le client devra confirmer avant la clôture de la mission.')">
        ...
        @elseif($mission->status == 'in_progress' && $mission->serviceRequest?->status === 'awaiting_validation')
        {{-- Client: validate the artisan's completion signal --}}
        <form action="{{ route('user.missions.update-status', $mission->id) }}" method="POST" class="flex flex-col md:flex-row gap-4 items-center"
            ...
            <button type="submit" class="w-full md:w-auto px-10 py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 justify-center">
                <i class="fas fa-check-circle"></i> Confirmer la fin du service
            </button>
        </form>
        @else
