...
<body class="h-full font-sans antialiased text-slate-800" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="px-8 py-10 border-b border-slate-100 bg-slate-50/30">
            <div class="flex items-center gap-4">
                <div class="relative flex-shrink-0 group">
                    <div
                        class="absolute -inset-1 bg-gradient-to-r from-rdc-blue to-blue-400 rounded-2xl blur opacity-25 group-hover:opacity-100 transition duration-500">
                    </div>
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'User') }}&background=007FFF&color=fff&size=128"
                        class="relative w-14 h-14 rounded-2xl border-2 border-white shadow-sm object-cover"
                        alt="Profile">
        ...
        <nav class="flex-1 overflow-y-auto px-6 py-8 space-y-10 custom-scrollbar">
            ...
            <div>
                <div
                    class="px-4 mb-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-heading flex items-center gap-3">
                ...
                <ul class="space-y-2">
                    <li>
                        <a href="{{ route('user.dashboard') }}"
                            class="flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold transition-all group {{ request()->routeIs('user.dashboard') ? 'bg-rdc-blue text-white shadow-xl shadow-blue-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <i
                                class="fas fa-border-all text-lg {{ request()->routeIs('user.dashboard') ? 'text-white' : 'text-slate-400 group-hover:text-rdc-blue' }}"></i>
                            Vue d'ensemble
                        </a>
                    </li>
                    <li>
                        <a href="#"
                            class="flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold transition-all group text-slate-500 hover:bg-slate-50 hover:text-slate-900">
                ...
                <ul class="space-y-2">
                    <li>
                        <a href="{{ route('user.services.index') }}"
                            class="flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold transition-all group {{ request()->routeIs('user.services.index') ? 'bg-rdc-blue text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <i
                                class="fas fa-wand-magic-sparkles text-lg {{ request()->routeIs('user.services.index') ? 'text-white' : 'text-slate-400 group-hover:text-rdc-blue' }}"></i>
                            Découvrir Services
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('user.jobs.index') }}"
                            class="flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold transition-all group {{ request()->routeIs('user.jobs.index') ? 'bg-rdc-blue text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <i
                                class="fas fa-briefcase text-lg {{ request()->routeIs('user.jobs.index') ? 'text-white' : 'text-slate-400 group-hover:text-rdc-blue' }}"></i>
                            Trouver un Job
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('user.career-advisor.index') }}"
                            class="flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold transition-all group {{ request()->routeIs('user.career-advisor.index') ? 'bg-rdc-blue text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <i
                                class="fas fa-magic text-lg {{ request()->routeIs('user.career-advisor.index') ? 'text-white' : 'text-slate-400 group-hover:text-amber-500' }}"></i>
                            Conseiller Carrière
                        </a>
                    </li>
                    <li>
                ...
                <ul class="space-y-2">
                    <li>
                        <a href="{{ route('user.messages.index') }}"
                            ...
                            Boîte de réception
                            <span
                                class="ml-auto bg-rdc-red text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-lg shadow-lg shadow-red-500/20">2</span>
                        </a>
                    </li>
                    <li>
                        <a href="#"
